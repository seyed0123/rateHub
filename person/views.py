...
from .models import person, Follower, Rate
...
class rate(View):
    def post(self, request, *args, **kwargs):
        ...
        global IDs
        username = IDs.get(token, 0)[0]
        rater = person.objects.get(username=username).id
        user = data['user']
        rate = data['rate']
        ...
        if user == rater:
            ...
            new_rater = Rate.objects.create(rater=rater, user=user, rate=rate)
            new_rater.save()
            return JsonResponse({'message': 'rated'})
